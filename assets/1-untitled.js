...
//    var dist = Math.sqrt(Math.pow(this.y, 2) + Math.pow(this.x, 2));
    ...
    this.color = Math.random() > .5 ? '#000' : '#'+Math.floor(Math.random()*16777215).toString(16); // elegant random color value code from http://www.paulirish.com/2009/random-hex-color-code-snippets/
}
...
Brush.prototype.draw = function ()
{
    if (this.alive) {
        ...
        ctx.arc(this.x, this.y, dist / 150, 0, 2*Math.PI);
